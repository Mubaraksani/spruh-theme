...
export function switcherArrowFn() {
  ...
  function slideClick() {
    ...
    let menuNav = document.querySelectorAll<HTMLElement>('.nav-sub');
    NavItem.forEach((element, index) => {
      if (element.classList.contains('show') == true) {
        element.classList.remove('show');
      }
    });
    menuNav.forEach((element, index) => {
  ...
  fromEvent(slideLeft, 'click').subscribe(() => {
    ...
    let menuNav: any = document.querySelector<HTMLElement>('.menu-nav');
    let mainContainer1: any =
      document.querySelector<HTMLElement>('.main-container-1');
  ...
  fromEvent(slideRight, 'click').subscribe(() => {
    let menuNav: any = document.querySelector<HTMLElement>('.menu-nav');
    let mainContainer1: any = document.querySelector<HTMLElement>('.main-container-1');
    let marginLeftValue = Math.ceil(Number(window.getComputedStyle(menuNav).marginLeft.split('px')[0]));
    let marginRightValue = Math.ceil(Number(window.getComputedStyle(menuNav).marginRight.split('px')[0]));
